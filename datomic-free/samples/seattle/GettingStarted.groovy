...
println "Using a pull expression to get entities' attributes and values, displaying first..."

pull_result = Peer.query("[:find (pull ?c [*]) :where [?c :community/name]]", conn.db())
println pull_result.get(0).get(0)
...
println "Find all communities and specify returning their names..."

results = Peer.query("[:find [?n ...] :where [_ :community/name ?n]]", conn.db())
for (result in results) println(result)

println "Find all community names and pull their urls..."

results = Peer.query("[:find ?n (pull ?c [:community/url]) :where [?c :community/name ?n]]", conn.db())
for (result in results) println(result)
...
results = Peer.query('''[:find [?c_name ...]
                     :where
...
results = Peer.query('''[:find ?c_name ?r_name
                     :where
                     [?c :community/name ?c_name]
                     [?c :community/neighborhood ?n]
                     [?n :neighborhood/district ?d]
...
results = Peer.query('''[:find ?n ?t :in $ [?t ...]
                     :where
                     [?c :community/name ?n]
                     [?c :community/type ?t]]''',
                 conn.db(),
...
results = Peer.query('''[:find ?n ?t ?ot
                     :in $ [[?t ?ot]]
                     :where
                     [?c :community/name ?n]
                     [?c :community/type ?t]
...
results = Peer.query('''[:find ?name ?cat
                     :in $ ?type ?search
                     :where
...
println Peer.query("[:find ?c :where [?c :community/name]]", db_asOf_schema).size()

println "\nFind all communities as of seed data transaction..."
db_asOf_data = conn.db().asOf(data_tx_date)
println Peer.query("[:find ?c :where [?c :community/name]]", db_asOf_data).size()

println "\nFind all communities since schema transaction..."
db_since_schema = conn.db().since(schema_tx_date)
println Peer.query("[:find ?c :where [?c :community/name]]", db_since_schema).size()

println "\nFind all communities since seed data transaction..."
db_since_data = conn.db().since(data_tx_date)
println Peer.query("[:find ?c :where [?c :community/name]]", db_since_data).size()
...
println Peer.query("[:find ?c :where [?c :community/name]]", db_if_new_data).size()

println "\nFind all communities currently in database..."
println Peer.query("[:find ?c :where [?c :community/name]]", conn.db()).size()
...
belltown_id = Peer.query("[:find ?id . :where [?id :community/name \"belltown\"]]", conn.db())
...
easton_id = Peer.query("[:find ?id . :where [?id :community/name \"Easton\"]]",
                 conn.db());
